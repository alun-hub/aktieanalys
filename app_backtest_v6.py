def run_strategy_backtest(symbols_dict, years=1, capital=100, pos_size=10, config=None):
    ...
    all_data = {}
    print(f"Backtest: Hämtar data för {len(symbols_dict)} symboler...")
    ...
    while current_date < end_dt:
        ...
        # Uppdatera marknadsdata för denna vecka
        ...
        # 2. Köp så många vi har råd med (upp till cash-gräns)
        available_slots = int(cash // (pos_size + calc_fee(pos_size)))
        if available_slots > 0:
            ...
            if candidates:
                ...
                # Köp de bästa kandidaterna vi har råd med denna vecka
                for i in range(min(len(candidates), available_slots)):
                    ...
                    if not tue.empty:
                        ...
                        holdings.append({"symbol": cand["sym"], "entry_price": entry, "sl": sl, "tp": tp, "qty": pos_size / entry, "entry_date": tue.index[0], "hwm": entry, "buy_fee": buy_fee})
                        cash -= (pos_size + buy_fee)
                        print(f"Backtest: Köpte {cand['sym']} @ {entry} (Score: {cand['score']})")
    ...
    print(f"Backtest klart. Totalt {len(trades)} affärer.")
