def run_strategy_backtest(symbols_dict, years=1, capital=100, pos_size=10, config=None):
    ...
    while current_date <= end_dt:
            ...
            if exit_type != "hold":
                ...
                if h["sl"] and row.Low <= h["sl"]: exit_p, exit_r = h["sl"], "Stop-Loss"
                    ...
                    if not pd.isna(row.MA10) and row.Close > row.MA10:
                        h["tp"], h["sl"] = None, row.MA10 # Trend riding
        ...
        cur_val = cash + sum(h["qty"] * float(all_data[h["symbol"]][all_data[h["symbol"]].index <= current_date].Close.iloc[-1]) for h in holdings if not all_data[h["symbol"]][all_data[h["symbol"]].index <= current_date].empty)
        target_pos = cur_val * pos_size_pct
        ...
        if last_buy_week != current_week and cash >= target_pos + calc_fee(target_pos):
            ...
            if candidates:
                ...
                holdings.append({"symbol": best["sym"], "entry_price": entry, "sl": sl, "tp": tp, "qty": target_pos / entry, "entry_date": current_date, "hwm": entry, "buy_fee": fee})
                cash -= (target_pos + fee)
    ...
    return {"final_value": round(cur_val, 2), "total_return_pct": round((cur_val/capital - 1) * 100, 2), "trades": trades, "history": history, "win_rate": round(len([t for t in trades if t["pl"] > 0]) / len(trades) * 100, 1) if trades else 0}
