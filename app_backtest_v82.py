        ...
        for h in holdings:
            ...
            if row.Low <= h["sl"]: exit_p, exit_r = h["sl"], "Stop-Loss (3xATR)"
                ...
                try:
                    ...
                    score = calc_swing_score(past, gen_signal(past), m_data)["score"]
        ...
        if cash >= target_pos + comm_val:
            ...
            for sym, df in all_data.items():
                ...
                try:
                    score = calc_swing_score(past, gen_signal(past), m_data)["score"]
                    if score >= min_score:
                        candidates.append({"sym": sym, "score": score, "atr": past.ATR.iloc[-1]})
                except: continue
            ...
            if candidates:
                ...
                best = candidates[0]
                # Hämta raden säkert
                day_rows = all_data[best["sym"]][all_data[best["sym"]].index == current_date]
                if not day_rows.empty:
                    entry = float(day_rows.iloc[0].Close)
