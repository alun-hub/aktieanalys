def run_strategy_backtest(symbols_dict, years=1, capital=100, pos_size=10, config=None):
    ...
    all_data = {}
    print(f"Backtest PRO v4.1: Hämtar data...")
    ...
    while current_date <= end_dt:
        ...
        if current_date.weekday() == 0 and cash >= target_pos_size_sek + calc_fee(target_pos_size_sek):
            ...
            for sym, df in all_data.items():
                ...
                try:
                    stock_mom, idx_mom = past_df.Mom_6m.iloc[-1], m_data["idx_mom_6m"]
                    if idx_mom > 0 and stock_mom < idx_mom * 2: continue
                    bb_w, bb_w_min, bb_w_max = past_df.BB_width.iloc[-1], past_df.BB_width.tail(100).min(), past_df.BB_width.tail(100).max()
                    bb_rank = (bb_w - bb_w_min) / (bb_w_max - bb_w_min + 1e-9)
                    sw = calc_swing_score(past_df, gen_signal(past_df), m_data)
                    if bb_rank < 0.20: sw["score"] += 3
                    if sw["score"] >= min_score:
                        candidates.append({"sym": sym, "score": sw["score"], "sltp": calc_sltp(past_df, float(past_df.Close.iloc[-1])), "atr": past_df.ATR.iloc[-1]})
                except: continue
            ...
            if candidates:
                ...
                if not row.empty:
                    ...
                    sl, tp = None, None
                    if exit_type == "sltp" or exit_type == "trailing_atr":
                        sl, tp = best["sltp"]["stop_loss"], best["sltp"]["take_profit_2"]
                        if exit_type == "trailing_atr": tp = None # Rid trenden
        ...
        if current_date.weekday() == 0: history.append({"date": current_date.strftime("%Y-%m-%d"), "value": round(cur_val, 2)})
