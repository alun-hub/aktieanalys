def run_strategy_backtest(symbols_dict, years=1, capital=100, pos_size_val=10, config=None):
    ...
    # Bestäm om pos_size är % eller SEK
    if pos_size_val <= 100:
        pos_size_pct = pos_size_val / 100.0
        use_pct = True
    else:
        fixed_pos_size = pos_size_val
        use_pct = False
    ...
    while current_date <= end_dt:
        ...
        for h in holdings:
            ...
            rows = df[df.index <= current_date].tail(1)
        ...
        if (current_date - last_buy_date).days >= 7 and cash >= target_pos + comm_val:
            ...
            if candidates:
                ...
                print(f"DEBUG: Köpte {best['sym']} @ {entry}")
